...
## Skill tiers (entry-level SWE)

**Baseline (screened on):** Git (branch/merge/PR/conflicts) · a first language (JS 66% / Python 57.9%, SO2024-25) · SQL (PostgreSQL now #1 DB) · HTML/CSS · TypeScript (now near-baseline, ~43%) · CS fundamentals (Big-O, arrays, linked lists, stacks, queues, hash tables, recursion) · HTTP/REST basics · unit testing basics.

**Differentiators:** TS depth (generics, strict) · Java/C# (enterprise) · Go (fast-growing) · React · Node + one backend framework · hands-on PostgreSQL/MySQL · a NoSQL (Mongo) · AWS basics · Docker (crossing to baseline) · CLI proficiency · reading existing code · Agile awareness · communication.
...
| Algorithmic / DSA (LeetCode-style) | ~70-80% big tech, ~40-50% non-tech | Dominant filter at large tech |
| Live technical screen | ~60-70% | Often paired with DSA |
| Take-home project | ~40-60% | Common at startups/mid-size |
| Pair programming | ~20-35% | Where LeetCode seen as unreliable |
| System design (junior) | ~15-25% | Scaled down ("design a URL shortener") |
...
## Specialization tracks (entry)
- **Frontend:** HTML/CSS/JS → TS → React (market leader) → responsive, a11y, Core Web Vitals, Tailwind. ~3-6 mo cited (optimistic). ✅
- **Backend:** one language (Python/Java/Node/Go) → SQL (PostgreSQL) → REST API design → auth (JWT/OAuth) → Git/CLI. 6mo-2yr. ✅
- **Full-stack:** front + back basics; meta-frameworks (Next.js/Django/Rails). Junior full-stack usually skews frontend-heavy. Most common startup title. ⚠️
- **Mobile:** Android (Kotlin + Jetpack Compose), iOS (Swift + SwiftUI), cross-platform (React Native, Flutter). Narrower market than web. ⚠️
